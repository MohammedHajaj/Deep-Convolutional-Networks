...
void InitializeTrainingData(unsigned int **P_Height, unsigned int **P_Width, size_t **P_Start, int **P_T, int **P_PStart, size_t **P_PartSize, size_t **P_StartPart, int *NParts, int *VParts, int Size)
{
	...
	*/
	...
	P_HeightTr :-    Pointer to the GPU buffer that will be allocated and initialized with the heights of the training\validation or test images.
	P_WidthTr :-     Pointer to the GPU buffer that will be allocated and initialized with the widths of the training\validation or test images.
	P_StartTr :-     Pointer to the GPU buffer that will be allocated and initialized with the starting positions the training\validation or test images.
	P_T :-           pointer to the GPU buffer that will be allocated and initialized with the labels of the training\validation or test images.
...
void InitialzeCuDNN_Var(cudnnTensorDescriptor_t Desc_X[NumWin][CL], cudnnTensorDescriptor_t Desc_Y[NumWin][CL], cudnnFilterDescriptor_t *Desc_W, cudnnConvolutionDescriptor_t *Conv_Desc, cudnnTensorDescriptor_t Desc_Xs[NumWin][CL], cudnnTensorDescriptor_t Desc_Ys[NumWin][CL], cudnnFilterDescriptor_t *Desc_Ws, cudnnConvolutionDescriptor_t *Conv_s_Desc, cudnnConvolutionFwdAlgo_t *FwdAlg, cudnnConvolutionBwdDataAlgo_t *BwdDataAlg, cudnnConvolutionBwdFilterAlgo_t *BwdFilterAlg, int *InCh, Var_Param *P)
{
	//----------------------------------------------------------------------------------------------------------------------------------------------
	/*
	...
	required to setup and initilize the convolutional layers. Here the tensor descriptors at each conv layer have multiple sizes because the 
	network is trained using a variable input size.
	*/
	...
	P:-             input Var_Param c struct that cchannel size.
	...
	*/
	...
	for (int i = 0; i < NumWin; i++)
		for (int j = 0; j < CL; j++)
		{
			dnnStatus = cudnnCreateTensorDescriptor(&Desc_X[i][j]);
			dnnStatus = cudnnCreateTensorDescriptor(&Desc_Y[i][j]);

			dnnStatus = cudnnSetTensor4dDescriptor(Desc_X[i][j], CUDNN_TENSOR_NCHW, CUDNN_DATA_FLOAT, BatchSize, InCh[j], P[i].IR[j], P[i].IR[j]);

			int batch, ch, h, w;
			dnnStatus = cudnnGetConvolution2dForwardOutputDim(Conv_Desc[j], Desc_X[i][j], Desc_W[j], &batch, &ch, &h, &w);

			dnnStatus = cudnnSetTensor4dDescriptor(Desc_Y[i][j], CUDNN_TENSOR_NCHW, CUDNN_DATA_FLOAT, batch, ch, h, w);
	...
	for (int i = 0; i < NumWin; i++)
		for (int j = 0; j < CL - JMP; j += JMP)
		{
			if (OutCh[j] != OutCh[j + JMP])
			{
				dnnStatus = cudnnCreateTensorDescriptor(&Desc_Xs[i][j]);
				dnnStatus = cudnnCreateTensorDescriptor(&Desc_Ys[i][j]);
				dnnStatus = cudnnSetTensor4dDescriptor(Desc_Xs[i][j], CUDNN_TENSOR_NCHW, CUDNN_DATA_FLOAT, BatchSize, OutCh[j], P[i].SR[j], P[i].SR[j]);

				int batch, ch, h, w;
				dnnStatus = cudnnGetConvolution2dForwardOutputDim(*Conv_s_Desc, Desc_Xs[i][j], Desc_Ws[j], &batch, &ch, &h, &w);
				dnnStatus = cudnnSetTensor4dDescriptor(Desc_Ys[i][j], CUDNN_TENSOR_NCHW, CUDNN_DATA_FLOAT, batch, ch, h, w);
			}
		}
...
void AllocateParamGPUMemory_Varm(float ***P_W, float ***P_V, float ***P_DW, float ***P_Ws, float ***P_Vs, float ***P_DWs, float ***P_X, float ***P_Y, float ***P_Param, float ***P_DParam, float ***P_ParamV, float ***P_SMU, float ***P_Derv, float **P_WF, float **P_VF, float **P_DWF, float **P_YF, float **P_Yv, int ***P_Indx, bool ***P_F, int *WSize, int *WsSize, Var_Param *P, int *Xc, int *Yc)
{
	//----------------------------------------------------------------------------------------------------------------------------------------------
	/*
	...
	of the squared derivatives. Here the buffers that hold the input channels and the output channels for each conv layer have multiple sizes
	because the network is trained using a variable input size.
	*/

	/****Argument List****/
	/*

	P_W:-      pointer ot a GPU buffer that will be allocated to hold the weights of all convolutional layers.
	P_V:-      pointer ot a GPU buffer that will be allocated to hold the running average of the squared derivatives of the weights of all convolutional layers.
	P_DW:-     pointer ot a GPU buffer that will be allocated to hold the current derivatives of the weights of all convolutional layers.
	P_Ws:-     pointer ot a GPU buffer that will be allocated to hold the weights of all convolutional layers for residual connections.
	P_Vs:-     pointer ot a GPU buffer that will be allocated to hold the running average of the squared derivatives of the weights of all convolutional layers for residual connections.
	P_DWs:-    pointer ot a GPU buffer that will be allocated to hold the current derivatives of the weights of all convolutional layers for residual connections.
	P_X:-      pointer ot a GPU buffer that will be allocated to hold the input channels of all convolutional layers.
	P_Y:-      pointer ot a GPU buffer that will be allocated to hold the output channels of all convolutional layers.
	P_Param:-  pointer ot a GPU buffer that will be allocated to hold the BN trainable parameters.
	P_ParamV:- pointer ot a GPU buffer that will be allocated to hold the running average of the squared derivatives of the BN trainable parameters.
	P_DParam:- pointer ot a GPU buffer that will be allocated to hold the current derivatives of the BN trainable parameters.
	P_SMU:-    pointer ot a GPU buffer that will be allocated to hold the current means and variances of all BN stages.
	P_Derv:-   pointer ot a GPU buffer that will be allocated to hold an intermediate values used to backpropagate the error signal through BN stages.
	P_WF:-     pointer ot a GPU buffer that will be allocated to hold the weights of the FC output layer.
	P_VF:-     pointer ot a GPU buffer that will be allocated to hold the running average of the squared derivatives of the weights of the FC output layer.
	P_DWF:-    pointer ot a GPU buffer that will be allocated to hold the current derivatives of the weights of the FC output layer.
	P_YF:-     pointer ot a GPU buffer that will be allocated to hold the outputs of the output layer.
	P_F:-      pointer ot a GPU buffer that will be allocated to hold the signs of the combined convolutional layer/residual connection output.
	P_Indx:-   pointer ot a GPU buffer that will be allocated to hold the indices of the maximum values of the maxpooling stages.
	WSize :-   input buffer that holds the number of weights in all conv layer.
	WsSize:-   output buffer that is used to calculate the number of weights for conv layers of the resdiual connections.
	P:-        is an input Var_Param struct that holds varaible conv layer parameters.
	Xc,Yc:-    paramters that defines the buffer sizes of the input and output channels of conv layers.
	*/
	//----------------------------------------------------------------------------------------------------------------------------------------------
	...
	for (int i = 0; i < CL; i++)
	{
		...
		cudaStatus = cudaMalloc(d_X + i, sizeof(float)*P[NumWin - 1].Xr[i] * Xc[i]);
		cudaStatus = cudaMalloc(d_Y + i, sizeof(float)*P[NumWin - 1].Yr[i] * Yc[i]);
	...
	*P_Ws = d_Ws; *P_Vs = d_Vs; *P_DWs = d_DWs;
	...
	for (int i = 0; i < CL; i++)
	{
		if (PoolType[i] == 1)
		{
			cudaStatus = cudaMalloc(d_Indx + i, sizeof(int)*P[NumWin - 1].SR[i] * P[NumWin - 1].SR[i] * BatchSize * OutCh[i]);
		}
	}
	...
	{
		cudaStatus = cudaMalloc(d_F + i, sizeof(bool)*P[NumWin - 1].Yr[i] * Yc[i]);
	}
...
void AllocateAuxiliaryGPUMemory_Varm(float ***P_SMUs, float **P_YY, float **P_Y0, float **P_ws, float **P_rand1, float **P_randRGB, float **P_Cropf, unsigned int **P_Crop, float **P_mse, float **P_count, int **P_Indx1, int ***PIndx, int *PStart, Var_Param *P, int *Yc, int NParts)
{
	//----------------------------------------------------------------------------------------------------------------------------------------------
	/*
	...
	*/
	...
	for (int i = 0; i < CL; i++)
	{
		if (P[NumWin - 1].Yr[i] * Yc[i] > size)size = P[NumWin - 1].Yr[i] * Yc[i];
	}
	...
	cudaStatus = cudaMalloc(&d_ws, sizeof(float)*wsSize);
...
void InitializeConvLayerParam_Var(Var_Param *P, int *InCh, int *Xc, int *Yc, int *WSize, float **Buffer)
{
	//----------------------------------------------------------------------------------------------------------------------------------------------
	/*
	The InitializeConvLayerParam() initializes the following variables which define the structure of the convolutional layers. Here some conv layer
	parameters have multiple sizes because the network is trained using a variable input size.
	*/
	...
	P[k].IR[i] :-   the height / width of a single square input channel for convolutional layer i and for the kth input size.
	P[k].CR[i] :-   the height / width of a single square output channel before maxpooling for convolutional layer i and for the kth input size.
	P[k].SR[i] :-   the height / width of a single square output channel after maxpooling for convolutional layer i and for the kth input size.
	P[k].Xr[i], Xc[i] :- where P[k].Xr[i]*Xc[i] defines the total size of the input channels for conv layer i and for the kth input size.
	P[k].Yr[i], Yc[i] :- where P[k].Xr[i]*Xc[i] defines the total size of the output channels for conv layer i and for the kth input size.
	WSize[i] :-     Number of weights in convolutional layer i.
	InCh[i] :-      number of input channels for convolutional layer i.
	Buffer:-        temporary main memory buffer used to swap data between main memory and GPU memory.
	...
	*/
	...
	for (int i = 0; i < NumWin; i++)
	{
		...
		for (int j = 0; j < CL; j++)
		{
			if (j == 0) P[i].CR[j] = (P[i].IR[j] + 2 * PAD[j] - Kr[j] + 2) / 2;
			else        P[i].CR[j] = P[i].IR[j] + 2 * PAD[j] - Kr[j] + 1;

			P[i].SR[j] = (PoolType[j] == 0) ? P[i].CR[j] : (P[i].CR[j] + Sr1[j] - 1) / Sr1[j];
			if (j == CL - 1)P[i].SR[j] = 1;
			...
			P[i].Xr[j] = P[i].IR[j] * P[i].IR[j];
			P[i].Yr[j] = P[i].CR[j] * P[i].CR[j];
		}
	}
...
void ParameterInitialization(float **d_W, float **d_Ws, float *d_WF, float **d_Param, float *Buffer, int *WSize, int *WsSize)
{
	//----------------------------------------------------------------------------------------------------------------------------------------------
	/*
	This function initializes the trainable parameters of the network. All weight are initialized using Kaiming initialization. The BN trainable
...
void InitializeCudaKernels_Var(Var_gridSizes *G, dim3 *gridSizeBN1, dim3 *gridSizeBN11, dim3 *gridSizeAddA, dim3 *P_gridSizeRGB, dim3 *P_gridSizePA, int *gridSizeAddWs, int *gridSizeAddW, int *P_gridSizeAddWF, Var_Param *P, int *Yc, int *WSize, int *WsSize)
{
	//----------------------------------------------------------------------------------------------------------------------------------------------
	/*
	This function initializes the following grid sizes of the main cuda kernels used to implement a deep residual CNN. Here some cuda kernels have
	multiple sizes because the network is trained using a variable input size.
	*/
	...
	G[k].gridSizeAddYB[i] :- the grid size used by the AddMatrix kernel which adds two matrecies with equal sizes at layer i for the kth input size.
	...
	*/
	...
	for (int i = 0; i < NumWin; i++)
	{
		dim3 gcrop((P[i].IR[0] * P[i].IR[0] + BLOCKSIZE1 - 1) / BLOCKSIZE1, BatchSize, 1);
		G[i].gridSize_Crop = gcrop;

		for (int j = 0; j < CL; j++)
		{
			int sn = P[i].SR[j] * P[i].SR[j];

			dim3 gsizep((sn + BLOCKSIZE1 - 1) / BLOCKSIZE1, OutCh[j], BatchSize);
			G[i].gridSizeP[j] = gsizep;

			dim3 gsizebn2((P[i].Yr[j] + BLOCKSIZE4 - 1) / BLOCKSIZE4, OutCh[j], BatchSize);
			G[i].gridSizeBN2[j] = gsizebn2;

			if (j < CL - JMP && j%JMP == 0)
				G[i].gridSizeAddYB[j] = GridSizeAC(BLOCKSIZE1, P[i].Yr[j + JMP] * Yc[j]);
		}
	}
