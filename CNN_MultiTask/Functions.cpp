...
void DividDatasetIntoMultipleTasks(int **P_Indx1, int **PIndx1, int ****PIndx, int ***PCountImgeTask, int *TrainSizeM, int *ValSizeM, int *d_T, int *PStart, int NParts, int VParts)
{
	//----------------------------------------------------------------------------------------------------------------------------------------------
	/*
	This function allocates and initializes buffers that are used to divide the main dataset into NumTasks tasks, after the initialization process
	the PIndx buffer will store the image indices of each task in a separate sub-buffer, and the PCountImgeTask will store the image count for each task.
	both the image indices and image counts are stored for each data part that fits in main memory.
	*/

	/****Argument List****/
	/*

	PIndx1 :-  pointer to a buffer that will be allocated and used by the ReshuffleImagesMT() function to store the indices of batches of all tasks of a single data part arranged in a round robin fashion.
	P_Indx1 :- pointer to a GPU buffer that will be allocated and used to copy the indices in PIndx1 to the GPU.
	PIndx :-   pointer to a buffer that contains a sub-buffer per task and a sub-buffer per data part, it will be allocated and used to store the image indices for each task in a separate sub-buffer.
	PCountImgeTask :-  pointer to a buffer that contains a sub buffer per task, it will be allocated and used to store the image counts per task per data part.
	TrainSize :- is the number of training images in a single epoch calculated by this function, it can be slightly larger than the actual number TrainSize if tasks have different number of training images.
	ValSize :- is the number of val images in a single epoch calculated by this function, it can be slightly larger than the actual number ValSize if tasks have different number of val images.
	d_T :- input GPU buffer that holds the training and validation images.
	PStart :-    input buffer that holds the starting image index of all data segments.
	NParts :- number of training data segments.
	NParts :- number of validation data segments.
	...
	*/
	...
	int ***Indx;
	int **CountImgeTask;
	int *T = new int[TrainSize + ValSize];
	cudaStatus = cudaMemcpy(T, d_T, sizeof(int) * (TrainSize + ValSize), cudaMemcpyDeviceToHost);
	...
	for (int i = 0; i < NParts; i++)
	{
		Indx[i] = new int*[NumTasks];
	}

	for (int i = 0; i < NParts; i++)
	{
		int **PPtemp = Indx[i];
		for (int j = 0; j < NumTasks; j++) PPtemp[j] = new int[MaxPart];
	}

	CountImgeTask = new int*[NParts];
	for (int i = 0; i < NParts; i++)
	{
		CountImgeTask[i] = new int[NumTasks];
	}

	//cudaStatus = cudaMalloc(P_Indx1, sizeof(int) * 2 * MaxPart);
	//*PIndx1 = new int[2 * MaxPart];

	for (int p = 0; p < NParts; p++)
	{
		int **SubIndx = Indx[p];
		int *SubCount = CountImgeTask[p];
		for (int i = 0; i < NumTasks; i++) SubCount[i] = 0;
		for (int i = PStart[p]; i < PStart[p + 1]; i++)
		{
			int itemp = T[i] / NumClassesTask;
			SubIndx[itemp][SubCount[itemp]] = i - PStart[p];
			SubCount[itemp]++;
		}
	}

	*PIndx = Indx; *PCountImgeTask = CountImgeTask;

	MaxPart = 0;
	int trainSizeM = 0, valSizeM = 0;
	for (int p = 0; p < NParts; p++)
	{
		int max = 0;
		int *SubCount = CountImgeTask[p];
		...
		{
			if (SubCount[i]>MaxPart)MaxPart = SubCount[i];
			if (SubCount[i]>max)max = SubCount[i];
		}
		if (p < NParts - VParts)
		{
			trainSizeM += ((max + BatchSize) / BatchSize)*BatchSize*NumTasks;
		}
		else
		{
			valSizeM += ((max + BatchSize) / BatchSize)*BatchSize*NumTasks;
		}

	}

	MaxPart = ((MaxPart + BatchSize) / BatchSize)*BatchSize*NumTasks;

	cudaStatus = cudaMalloc(P_Indx1, sizeof(int) * MaxPart);
	*PIndx1 = new int[MaxPart];
	*TrainSizeM = trainSizeM;
	*ValSizeM = valSizeM;
	delete[]T;
}
...
void DividTestsetIntoMultipleTasks(unsigned int **P_flip, int **PIndx1, int ***Indx, int **CountImgeTask, int *d_T, int *PStart, int NParts, curandGenerator_t cuda_gen)
{
	//----------------------------------------------------------------------------------------------------------------------------------------------
	/*
	This function is similar to the DividDatasetIntoMultipleTasks() function, but it divides the main test set to multiple test sets, one test set per task.
	The input arguments are also similar, but they have already been allocated by the DividDatasetIntoMultipleTasks() function, and therefore they will only
	be initialized by the indices and image count of the test sets. One exception is the P_flip buffer which will be used to store random values to decide
	on horizontal flipping.
	*/
	//----------------------------------------------------------------------------------------------------------------------------------------------
	...
	int *T = new int[TestSize];
	cudaStatus = cudaMemcpy(T, d_T, sizeof(int) * TestSize, cudaMemcpyDeviceToHost);
	...
	for (int p = 0; p < NParts; p++)
	{
		...
		TestSizeM += ((max + BatchSize) / BatchSize)*BatchSize*NumTasks;
	}

	MaxPart = ((MaxPart + BatchSize) / BatchSize)*BatchSize*NumTasks;
	*PIndx1 = new int[MaxPart];

	cudaStatus = cudaMalloc(P_flip, sizeof(int) * TestSizeM*EpochTs);
	curandStatus = curandGenerate(cuda_gen, *P_flip, TestSizeM*EpochTs);
...
void ReshuffleImagesMT(int *partsize, int *Indx1, int ***Indx, int **CountImgeTask, int *PStart, int p)
{
	//----------------------------------------------------------------------------------------------------------------------------------------------
	/*
	This function randomly reshuffles the images of a single data part by reshuffling the images of each task separately. Then it generates one batch
	per task in a round robin fashion until to goes through the entire data part. The indices of the all images that make the arranged batches are then
	stored in the Indx1 buffer. This function uses the Indx and CountImgeTask buffers generated by the DividDatasetIntoMultipleTasks() function to generate
	the round robin arranged batches.
	*/
	//----------------------------------------------------------------------------------------------------------------------------------------------
	...
	{
		if (SubCount[ii]>MAX) MAX = SubCount[ii];
	}

	for (int ii = 0; ii < NumTasks; ii++)
		for (int j = SubCount[ii] - 1; j >= 1; j--)
		{
			int x = distr_int(gen) % j;
			int temp = SubIndx[ii][x];
			SubIndx[ii][x] = SubIndx[ii][j];
			SubIndx[ii][j] = temp;
		}

	for (int j = 0; j < NumTasks; j++) TCount[j] = 0;

	for (int ii = 0; ii < (MAX + BatchSize) / BatchSize; ii++)
	{
		for (int k = 0; k < NumTasks; k++)
		{
			for (int j = 0; j < BatchSize; j++)
			{
				Indx1[ii*NumTasks*BatchSize + k*BatchSize + j] = SubIndx[k][TCount[k] % SubCount[k]];
				TCount[k]++;
			}
		}
	}
	*partsize = ((MAX + BatchSize) / BatchSize)*BatchSize*NumTasks;
}

//============================================================================================================================================================


void SaveParameters1(FILE *out, float *TEMP, float **d_W, float **d_Ws, float *d_WF, float **d_Param, int *WSize, int *WsSize, int In1)
{
...
void ReloadParameters1(FILE *in, float *TEMP, float **d_W, float **d_Ws, float *d_WF, float **d_Param, int *WSize, int *WsSize, int In1)
{
	//----------------------------------------------------------------------------------------------------------------------------------------------
	/*
...
void SaveParameters2(FILE *out, float *TEMP, float **d_W, float **d_V, float **d_Ws, float **d_Vs, float *d_WF, float *d_VF, float **d_Param, float **d_ParamV, int *WSize, int *WsSize, int In1, int iter0, int Iter0, int kk0, float lr0)
{
	//----------------------------------------------------------------------------------------------------------------------------------------------
	/*
...
void ReloadParameters2(FILE *in, float *TEMP, float **d_W, float **d_V, float **d_Ws, float **d_Vs, float *d_WF, float *d_VF, float **d_Param, float **d_ParamV, int *WSize, int *WsSize, int In1, int *iter0, int *Iter0, int *kk0, float *lr0)
{
	//----------------------------------------------------------------------------------------------------------------------------------------------
	/*
...
void InitializeTrainingData(unsigned int **P_Height, unsigned int **P_Width, size_t **P_Start, int **P_T, int **P_PStart, size_t **P_PartSize, size_t **P_StartPart, int *NParts, int *VParts, int Size)
{
	//----------------------------------------------------------------------------------------------------------------------------------------------
	/*
	...
	*/
	...
	strcpy_s(FileName, 128, DataFloder); strcat_s(FileName, 128, "trainlabels_I.txt");
	fopen_s(&in5, FileName, "rb");
	strcpy_s(FileName, 128, DataFloder); strcat_s(FileName, 128, "cordinatesVal.txt");
	fopen_s(&in9, FileName, "rb");
	strcpy_s(FileName, 128, DataFloder); strcat_s(FileName, 128, "vallabels_I.txt");
	...
	if (Operation_Mode == INFERENCE || Size == (TrainSize + ValSize))
	{
	...
	if (Size == TestSize)
	{
		size_t numread1 = fread(Height, sizeof(unsigned int), TestSize, in9);
		size_t numread2 = fread(Width, sizeof(unsigned int), TestSize, in9);
		size_t numread3 = fread(T, sizeof(int), TestSize, in10);
	...
	else
	{
		...
		numread1 = fread(Height + TrainSize, sizeof(unsigned int), ValSize, in9);
		numread2 = fread(Width + TrainSize, sizeof(unsigned int), ValSize, in9);
		numread3 = fread(T + TrainSize, sizeof(int), ValSize, in10);
	}
	...
	*VParts = 0;
	...
	for (int i = 1; i <= Size; i++)
	{
		...
		{
			jj++;
			PStart[jj] = i - 1;//((i - 1) / NumClasses) * NumClasses; //
			PartSize[jj - 1] = Start[PStart[jj]] - Start[PStart[jj - 1]];
			StartPart[jj] = Start[PStart[jj]];

			if (Size == (TrainSize + ValSize) && i > TrainSize) (*VParts)++;
		}
		else
		{
			if (Size == TestSize)
			{
				...
				{
					jj++;
					PStart[jj] = i;//(i / NumClasses) * NumClasses; //
					PartSize[jj - 1] = Start[PStart[jj]] - Start[PStart[jj - 1]];
					StartPart[jj] = Start[PStart[jj]];
				}
			...
			{
				if (i == TrainSize || i == TrainSize + ValSize)
				{
					jj++;
					PStart[jj] = i;//(i / NumClasses) * NumClasses; //
					PartSize[jj - 1] = Start[PStart[jj]] - Start[PStart[jj - 1]];
...
void AllocateAuxiliaryGPUMemory(float ***P_SMUs, float **P_YY, float **P_Y0, float **P_ws, float **P_rand1, float **P_randRGB, float **P_Cropf, unsigned int **P_Crop, float **P_mse, float **P_count, int *PStart, int *Yr, int *Yc, int NParts, int TrainSizeM)
{
	//----------------------------------------------------------------------------------------------------------------------------------------------
	/*
	...
	*/
	...
	P_YY:-      pointer to a GPU buffer that will be allocated to hold intermidiate output channels between BN and maxpooling.
	P_Y0:-      pointer to a GPU buffer that will be allocated to hold the the inputs to output FC layer.
	P_ws:-      pointer to a GPU buffer that will be allocated to hold temp GPU memory that will be used by the cudnn convolution algorithms.
	P_rand1:-   pointer to a GPU buffer that will be allocated to hold random normal values that will be used by the RGBRandPCA cuda kernel for colour augmentation.
	P_randRGB:- pointer to a GPU buffer that will be allocated to hold stochastic values generated by the RGBRandPCA cuda kernel for colour augmentation.
	P_Crop:-    pointer to a GPU buffer that will be allocated to hold random integer values used to decide croping positions and probability of horizontal flipping for data augmentation.
	P_Cropf:-   pointer to a GPU buffer that will be allocated to hold the input channels of all convolutional layers.
	P_mse:-     pointer to a GPU buffer that will be allocated to accumulate the mean square error for the training/validation sets.
	P_count:-   pointer to a GPU scalar to compute the number of correctly classified training/validation images.
	P_Indx1:-   pointer to a GPU buffer that will be allocated to hold image indices for a data segement generated by the resuffel algorithm.
	PIndx:-     pointer to a buffer that will be allocated to be used by the resuffel function.
	P_SMUs:-    pointer to a GPU buffer that will be allocated to hold the fixed means and variances of all BN stages that will be used in the inference stage.
	...
	*P_mse = d_mse; *P_count = d_count;
	...
	for (int i = 0; i < CL; i++)
	{
		cudaStatus = cudaMalloc(d_SMUs + i, sizeof(float) * 2 * OutCh[i] * NumTasks);
		cudaStatus = cudaMemset(d_SMUs[i], 0, sizeof(float) * 2 * OutCh[i] * NumTasks);
	}
...
void ParameterInitialization(float **d_W, float **d_Ws, float *d_WF, float **d_Param, float *Buffer, int *WSize, int *WsSize, int In1)
{
	//----------------------------------------------------------------------------------------------------------------------------------------------
	/*
...
void InitializeCudaKernels(dim3 *gridSizeP, dim3 *gridSizeBN1, dim3 *gridSizeBN11, dim3 *gridSizeBN2, dim3 *gridSizeAddA, dim3 *P_gridSizeRGB, dim3 *P_gridSize_Crop, dim3 *P_gridSizePA, int *gridSizeAddYB, int *gridSizeAddWs, int *gridSizeAddW, int *P_gridSizeAddWF, int *SR, int *Yr, int *Yc, int *WSize, int *WsSize, int TrainSizeM)
{
	//----------------------------------------------------------------------------------------------------------------------------------------------
	/*
...
void PrintIterResults(FILE *out, float *d_mse, float *d_count, int *PStart, int NParts, int VParts, int iter, int TrainSizeM, int ValSizeM)
{
	cudaError_t cudaStatus;
...
void InitializeMultiCropInference(int **P_MTX)
{
	...
	*/
	...
	unsigned int *Height = new unsigned int[TestSize];
	unsigned int *Width = new unsigned int[TestSize];
	size_t numread1 = fread(Height, sizeof(unsigned int), TestSize, in1);
	size_t numread2 = fread(Width, sizeof(unsigned int), TestSize, in1);
...
void PrintFinalResults(FILE *out1, float *d_Yss, float *d_mse, int *d_T)
{
	...
	float *mse = new float[BatchSize];
	float *Yss = new float[NumClasses*TestSize];
	int *T = new int[TestSize];

	cudaStatus = cudaMemcpy(mse, d_mse, sizeof(float)*BatchSize, cudaMemcpyDeviceToHost);
	cudaStatus = cudaMemcpy(Yss, d_Yss, sizeof(float) * NumClasses*TestSize, cudaMemcpyDeviceToHost);
	cudaStatus = cudaMemcpy(T, d_T, sizeof(int)*TestSize, cudaMemcpyDeviceToHost);

	float max1, max2;
	int indx1, indx2;
	int count[5] = {};
	int SubCount[NumTasks] = {}, SubCountT[NumTasks] = {};
	...
	for (int n = 0; n < 2; n++)
	{
		...
		for (int i = 0; i < TestSize; i++)
		{
			...
			{
				COUNT[indx1][indx2]++;
				int ix = indx1 / NumClassesTask;
				SubCountT[ix]++;
				if (indx1 == indx2)SubCount[ix]++;
			}
	...
	cout << endl << "mse= " << Error / float(EpochTs*TestSize) << endl << endl;
	fprintf(out1, " \n mse = %f \n\n", Error / float(EpochTs*TestSize));
	for (int i = 0; i < 2; i++)
	{
		cout << endl << "conut = " << count[i] << "         ratio = " << float(count[i]) / float(TestSize) << endl;
		fprintf(out1, " \n count = %d \t\t ratio = %f \n", count[i], float(count[i]) / float(TestSize));
	}
	...
	for (int i = 0; i < NumTasks; i++)
	{
		cout << endl << "conut = " << SubCount[i] << "         ratio = " << float(SubCount[i]) / float(SubCountT[i]) << endl;
		fprintf(out1, " \n count = %d \t\t ratio = %f \n", SubCount[i], float(SubCount[i]) / float(SubCountT[i]));
	}
