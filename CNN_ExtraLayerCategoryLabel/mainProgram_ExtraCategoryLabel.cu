...
//============================================================================================================================================================
// This CUDA program implements a deep residual CNN that uses a coarse category label in addition to the class label for each image. An extra FC output layer 
// that predicts the extra coarse category label is appended to the network at an earlier stage in comparison to the main FC output layer that predicts 
// the class label of the image which is often placed as the last layer in the network. In the 34-layer deep residual network this extra output layer is added 
// after conv layer 27. A global average pooling is applied to the output channels of conv layer 27 and the outputs of that pooling stage is feed as inputs to
// the extra added output layer. The location was chosen based on a t-SNE visualization obtained using a standard network trained using a hierarchical dataset 
// made of 10 coarse categories, and used class labels only. The visualization shows that the network learns to separate between the coarse categories at an 
// earlier stage while it only separates between individual classes at the latest stages in the network. Our results show that incorporating the coarse category 
// labels earlier than the class labels produces better results. Therefore, to get the benefits of this extra output layer, the training dataset has to be a 
// hierarchical dataset, where classes can be divided into coarse categories, and each image is asigned a class label and a category label. Finally, the extra
// output layer is not used in the inference stage, and therefore the network used for inference is a standard network.
//============================================================================================================================================================
...
// lmda is 0.0005 in this case when the batch size is 25.
//============================================================================================================================================================


float lr = 0.03f / float(BatchSize);
float lmda = 0.0125f;
...
char DataFloder[] = "E:/Data_100Divided/";
...
int main()
{
	...
	// d_T :-       GPU buffer stores the class labels. 
	// d_T2 :-      GPU buffer stores the coarse category labels. 
	...
	int *d_T, *d_T2, *PStart, NParts, VParts;
	size_t *d_StartTr, *PartSize, *StartPart;


	InitializeTrainingData_PlusCatLabel(&d_HeightTr, &d_WidthTr, &d_StartTr, &d_T, &d_T2, &PStart, &PartSize, &StartPart, &NParts, &VParts, TrainSizeM + ValSizeM);
	...
	// derivatives, and the running averages of the squared derivatives of all trainable parameters. It also allocates the buffers required to store 
	// the input and output channels of all conv layers. The definitions of these variables is provided in the Function.cpp file that includes the 
	// implementation of the AllocateParamGPUMemory() function. 
	//
	...
	//
	// The AllocateGPUMemory_ExtraOutputLayer function allocates the GPU memory required to store the parameters and outputs of the extra output layer
	// which is used to predict the coarse category label.
	//=======================================================================================================================================================	
	...
	AllocateParamGPUMemory(&d_W, &d_V, &d_DW, &d_Ws, &d_Vs, &d_DWs, &d_X, &d_Y, &d_Param, &d_DParam, &d_ParamV, &d_SMU, &d_Derv, &d_WF, &d_VF, &d_DWF, &d_YF, &d_Yv, &d_Indx, &d_F, WSize, WsSize, Xr, Xc, Yr, Yc, SR);

	float **d_SMUs, *d_YY, *d_Y0, *d_ws, *d_mse, *d_count, *d_rand1, *d_randRGB, *d_Cropf;
	int *Indx1, **Indx, *d_Indx1;
	unsigned int *d_Crop;

	AllocateAuxiliaryGPUMemory(&d_SMUs, &d_YY, &d_Y0, &d_ws, &d_rand1, &d_randRGB, &d_Cropf, &d_Crop, &d_mse, &d_count, &d_Indx1, &Indx, PStart, Yr, Yc, NParts);

	float *d_WF2, *d_VF2, *d_DWF2, *d_YF2, *d_XC, *d_mse2, *d_count2;

	AllocateGPUMemory_ExtraOutputLayer(&d_WF2, &d_VF2, &d_DWF2, &d_YF2, &d_XC, &d_mse2, &d_count2, Xr, Xc);
	...
	InitializeCudaKernels(gridSizeP, gridSizeBN1, gridSizeBN11, gridSizeBN2, gridSizeAddA, &gridSizeRGB, &gridSize_Crop, &gridSizePA, gridSizeAddYB, gridSizeAddWs, gridSizeAddW, &gridSizeAddWF, SR, Yr, Yc, WSize, WsSize);

	//-------------------------------------------------------------------------------------------------------------------------------------
	// The InitializeCudaKernels_ExtraOutputLayer function initializes the grid sizes for the cuda kernels 
	// required to incorporate the extra output layer.
	//
	// gridSizePA_Cat :-  the grid size for the global average pooling applied before adding the extra output layer.
	// gridSizeAdd_Cat :- the grid size used by the AddMatrix kernel to combine the error signals from the extra and the main output layers.  
	// gridSizeAddWF :-   the grid size for the cuda kernel used to update the weights of the extra FC output layer.
	//--------------------------------------------------------------------------------------------------------------------------------------

	dim3 gridSizePA_Cat; 
	int gridSizeAdd_Cat, gridSizeAddWF2;

	InitializeCudaKernels_ExtraOutputLayer(&gridSizePA_Cat, &gridSizeAdd_Cat, &gridSizeAddWF2, Xr, Xc);
	...
	// The ParameterInitialization_ExtraOutputLayer is used to initialize the parameters of the extra output layer.  
	//=======================================================================================================================================================	
	...
	if (Operation_Mode == TRAIN || Operation_Mode == TRAIN_PLUS_INFERENCE)
	{
		...
		ParameterInitialization_ExtraOutputLayer(d_WF2,TEMP);
	}
	...
	if (Operation_Mode == INFERENCE)
	{
		ReloadParameters1(inParam, TEMP, d_W, d_Ws, d_WF, d_Param, WSize, WsSize);
	}
	...
	if (Operation_Mode == INTERRUPTED_TRAIN || Operation_Mode == INTERRUPTED_TRAIN_PLUS_INFERENCE)
	{
		ReloadParameters2_PlusExtraLayer(inParam, TEMP, d_W, d_V, d_Ws, d_Vs, d_WF, d_VF, d_WF2, d_VF2, d_Param, d_ParamV, WSize, WsSize, &i0, &Iter0, &kk0, &lr);
	}
	...
	if (Operation_Mode == TRAIN || Operation_Mode == INTERRUPTED_TRAIN || Operation_Mode == TRAIN_PLUS_INFERENCE || Operation_Mode == INTERRUPTED_TRAIN_PLUS_INFERENCE)
	{
		...
		for (int i = i0; (i < NParts*NumEpoch) && MSE < 100; i++)
		{
			...
			for (int j = PStart[p]; j < PStart[p + 1]; j += BatchSize)
			{
				...
				for (int k = 0; k < CL; k++)
				{
					...
					//=========================================================================================================================================
					// The following if statement includes the code to implement the extra FC output layer which predicts the extra coarse category label.
					// This extra layer is added after conv layer k = K_Cat. First the GlobalAvgPoolingForward() cuda kernel is applied to the output channels 
					// of conv layer K_Cat, and the outputs of the pooling stage are the inputs for the extra FC output layer. Then the cublass.lib matrix
					// multiplication function cublasSgemm() implements the FC extra output layer. Finally a softmax (or log sigmoid) activation is applied. 
					// To simplify the code this block contains both the forward and backward passes for this extra layer. In the backward pass the cublasSgemm()
					// matrix multiplication function is used to calculate the derivatives of this extra FC layer, and to back-propagate the error signal. Then
					// the GlobalAvgPoolingBackward() cuda kernel propagates back the error signal through the added average pooling layer.
					//=========================================================================================================================================
					
					if (k == K_Cat)
					{
						GlobalAvgPoolingForward <BLOCKSIZE3> << <gridSizePA_Cat, blockSize3 >> >(d_YY, d_ts, OutCh[k], SR[k] * SR[k]);						

						blasstatus = cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, Out2, BatchSize, In2 + 1, &alpha1, d_WF2, Out2, d_YY, In2 + 1, &beta1, d_YF2, Out2);

						LogSigmoid<Out2, BLOCKSIZE2> << <BatchSize, BLOCKSIZE2 >> >(d_YF2, d_T2 + PStart[p], d_Indx1 + j - PStart[p], d_mse2, d_count2);
						//Softmax<Out2, BLOCKSIZE2> << <Batch, BLOCKSIZE2 >> >(d_YF2, d_T2 + PStart[p], d_Indx1 + j - PStart[p], d_mse2, d_count2);

						//-----------------------------------------------------------------------------------------------------------------------------------------
						// Back-propagation
						//-----------------------------------------------------------------------------------------------------------------------------------------

						blasstatus = cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_T, Out2, In2 + 1, BatchSize, &alpha1, d_YF2, Out2, d_YY, In2 + 1, &beta1, d_DWF2, Out2);

						blasstatus = cublasSgemm(handle, CUBLAS_OP_T, CUBLAS_OP_N, In2, BatchSize, Out2, &alpha1, d_WF2, Out2, d_YF2, Out2, &beta1, d_YY, In2 + 1);

						cudaStatus = cudaMemsetAsync(d_XC, 0, Xr[k + 1] * Xc[k + 1] * sizeof(float));
						
						GlobalAvgPoolingBackward <BLOCKSIZE3> << <gridSizePA_Cat, blockSize3 >> >(d_XC, d_YY, OutCh[k], SR[k] * SR[k]);
				...
				if (p < (NParts - VParts) && epoch < (NumEpoch - 1))
				{
					...
					for (int k = CL - 1; k >= 0; k--)
					{
						...
						if (k == K_Cat) Add_Mtx <1> << <gridSizeAdd_Cat, BLOCKSIZE1 >> >(d_ts, d_XC, Xr[k + 1] * Xc[k + 1]);
			...
			if ((epoch + 1) % 50 == 0) SaveParameters2_PlusExtraLayer(outParamCopy, TEMP, d_W, d_V, d_Ws, d_Vs, d_WF, d_VF, d_WF2, d_VF2, d_Param, d_ParamV, WSize, WsSize, i + 1, Iter, kk, lr);
		...
		//=========================================================================================================================================
		// The parameters of the extra output layers (d_WF2) are not saved because this layer is not used in the inference stage.
		//=========================================================================================================================================

		SaveParameters1(outParam, TEMP, d_W, d_Ws, d_WF, d_Param, WSize, WsSize);
		fclose(outParam);
		fclose(outParamCopy);
	...
	//                                                       Inference phase.
	//
	// The extra output layer that predicts the coarse category label is not used in the inference stage, and therefore the network used for inference is a standard network,
	// and the cuda code is the same code used to implement the inference stage for a standard deep residual network trained using class labels only.
	//
	//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	if (Operation_Mode == INFERENCE || Operation_Mode == TRAIN_PLUS_INFERENCE || Operation_Mode == INTERRUPTED_TRAIN_PLUS_INFERENCE)
	{
		...
		InitializeTrainingData_PlusCatLabel(&d_HeightTr, &d_WidthTr, &d_StartTr, &d_T, &d_T2, &PStart, &PartSize, &StartPart, &NParts, &VParts, TestSizeM);
